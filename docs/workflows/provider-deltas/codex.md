...
### 2. Prompt Shape Stability Is Operational

对 Codex 来说，
prompt shape stability
不是纯成本优化，
而是长回合行为稳定性的组成部分。

默认要求：

1. static instructions、tool descriptors、
   sandbox / cwd / approval banner
   与 session-start metadata
   一起构成需要尽量稳定的 exact prefix
2. 新的 task delta、tool observation
   与最新 user intent
   默认追加在尾部，
   不静默回写前缀
3. mid-run 若切换 model、
   ...
   变更 sandbox mode、
   approval mode 或 cwd，
   应视为显式 runtime boundary，
   并在 handoff、recovery
   或 reviewable artifact 中写清

### 3. Subagent Use Should Stay Bounded
...
### 4. Worktree Escalation Is For Real Parallel Write Threads
...
### 5. State Mutation Must Prefer Repo Scripts
...
### 6. Autonomy Budget Must Be Explicit
...
### 7. Missing Adapter Capability Should Produce Honest Output
