...
它不定义 runtime/session trace，也不改写 `items/` 作为 source of truth 的地位。

## 核心原则

1. `items/` 仍然是运行态 source of truth。
2. `boards/` 仍然是 generated-only 视图。
3. board refresh event 记录的是派生视图写入，不是业务状态迁移。
4. board refresh event 必须独立于 work item transition ledger。
5. 只有发生实际 board 文件变化时才写 board refresh event。
...
3. `.harness/workspace/departments/<department>/workspace/board.md`

## 与 trace taxonomy 的关系

1. `docs/workflows/work-item-trace-taxonomy.md` 里的 `board-refresh` 仍然保留。
2. 但它不写进 item-scoped `.harness/workspace/state/transitions/`。
