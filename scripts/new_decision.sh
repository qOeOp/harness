...
usage() {
  cat <<EOF >&2
usage: $0 [--work-item <WI-xxxx>] [--promote-shared-writeback|--promote-governance] [shared] <title>

Decision packs default to task-local routing.
Pass --work-item explicitly for task-local routing.
Use --promote-shared-writeback (compat: --promote-governance) only for shared cross-task decisions in shared writeback mode.
`company` remains accepted as a compatibility alias for the same shared surface.
EOF
  exit 1
}
...
if [ "$promote_shared_writeback" -eq 1 ]; then
  ...
  case "$scope" in
    company|shared)
      require_shared_writeback_mode_for_workspace_artifact "decision pack" || exit 1
      target=".harness/workspace/decisions/log/${date}-${slug}.md"
      ;;
    *)
      echo "workstream-specific decision outputs are no longer part of the canonical runtime: $scope" >&2
      exit 1
      ;;
  esac
else
