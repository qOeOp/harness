...
run_core_checks() {
  ...
  check_file ".agents/skills/harness/SKILL.md"
...
run_governance_checks() {
  ...
  check_optional_compat_dir ".harness/workspace/state/items" ".harness/workspace/state/items/README.md" "Compatibility only: true"
  check_optional_compat_dir ".harness/workspace/state/progress" ".harness/workspace/state/progress/README.md" "Compatibility only: true"
  ...
  for file in .claude/commands/*.md .claude/skills/*/SKILL.md .agents/skills/harness/skills/*/SKILL.md; do
    [ -f "$file" ] || continue
    check_file "$file"
  done
  ...
  if git rev-parse --git-dir >/dev/null 2>&1; then
    ...
    if [ "$hooks_path" != ".githooks" ]; then
      fail_message "git core.hooksPath is not set to .githooks"
    fi
  fi
