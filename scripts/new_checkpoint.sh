...
usage() {
  cat <<EOF >&2
usage: $0 [--work-item <WI-xxxx>] [--promote-shared-writeback|--promote-governance] <label>

Pass --work-item explicitly for task-local routing.
Checkpoints default to task-local attachments.

Use --promote-shared-writeback (compat: --promote-governance) only for cross-task snapshots in shared writeback mode.
EOF
  exit 1
}
