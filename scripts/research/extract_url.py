...
def to_markdown(payload: dict) -> str:
    ...
    return "\n".join(
        [
            ...
            f"- Origin status: {payload['origin_status'] if payload['origin_status'] is not None else 'n/a'}",
            f"- Content-Type: {payload['content_type']}",
            f"- Retrieved at: {payload['retrieved_at']}",
            f"- Title: {title}",
            f"- Cache: {payload['cache_status'] or 'n/a'}",
            f"- Served from cache: {'yes' if payload['served_from_cache'] else 'no'}",
            f"- Attempts: {payload['attempts']}",
            f"- ETag: {payload['etag'] or 'n/a'}",
            f"- Last-Modified: {payload['last_modified'] or 'n/a'}",
