#!/bin/sh
# Canonical surface diagnostic entrypoint.
# Legacy compatibility alias: run_governance_surface_diagnostic.sh
set -eu
...
if [ "$mode" = "source" ]; then
  ...
  legacy_state_item_refs=$(rg -l '\.harness/workspace/state/items/' README.md docs scripts roles skills references --glob '!references/archive/**' --glob '!scripts/run_surface_diagnostic.sh' 2>/dev/null | wc -l | tr -d ' ')
  shared_workspace_refs=$(rg -l '\.harness/workspace/(current|briefs|state)/' README.md docs scripts roles skills references --glob '!references/archive/**' --glob '!scripts/run_surface_diagnostic.sh' 2>/dev/null | wc -l | tr -d ' ')
...
2. 若要把结果沉淀成周期性 surface audit，可运行：
   \`./scripts/run_surface_diagnostic.sh --mode consumer --write .harness/workspace/status/process-audits/\$(date +%F)-surface-diagnostic.md\`
