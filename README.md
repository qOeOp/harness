...
9. eval bootstrap 不必一开始就追求大而全；
   先用 20-50 个来自真实失败、
   真实工单或代表性边界条件的 case
   立起 capability slice，通常更稳
10. agent eval 的每次 trial
    默认应从干净、隔离的环境启动；
    不让 cache、残留文件、历史 git 状态
    或共享资源抖动污染结果
11. capability eval 与 regression eval
    默认分层；
    前者负责 hill-climbing，
    后者负责守住已达成行为，
    成熟 capability case
...
15. 不是 service-provider-operated 的
...
23. durable state 与 serialized context
    必须带版本与迁移边界
...
47. dynamic hook / session-start context
    只放快变环境事实；
    若影响恢复、审批或外部承诺，
    必须回写 canonical task surface
48. `MCP roots` 用于 scope /
    discovery，不是硬安全边界；
    enforcement 仍在 sandbox、
    approval 与 access control
49. telemetry semconv 与 replay tooling
    也要版本化；
    不静默用新字段语义
    重解释旧 trace
