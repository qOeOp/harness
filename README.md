...
9. skill 默认属于 versioned instruction surface，
   不是自动获得高优先级的 enforced policy channel；
   `allowed-tools`、`model`、`context: fork`、
   `disable-model-invocation` 这类 metadata
   也会影响 capability / invocation，但本质仍是 guidance，不是机械约束
10. 若某条规则的违规成本高，
    必须继续下沉到
...
3. 若 source 变更导致 budget breach，
   ...
   只允许 `compress / merge / archive / delete`
   或必须的 bug fix / contract tightening /
   mechanical enforcement
4. 提高 budget 不是普通实现细节，
   而是 control-surface change；
   需要显式 reviewable rationale，
   不允许靠连续提交静默抬高
5. README 的增长也属于 active-surface entropy；
   若 README 变大但默认读序
   没更短、更稳、更少歧义，默认视为退化
...
7. README update 也受
   active-surface entropy budget 约束；
   不能把它当成无限扩写的缓冲层
...
- `provider-specific projection surface`
  ...
  `.claude/settings.json`、provider-specific hooks、
  custom commands、skill catalogs、
  MCP prompts / slash-command adapters
  更适合作为 delivery projection，
  不是新的 canonical truth；同一条规则若要跨 provider 生效，
  默认应优先维护一个 canonical source
  或可生成的 thin adapter，
  不手工养出长期漂移的平行 instruction truth
- `dynamic / path-scoped instruction loading`
  session-start hooks、nested rules、subtree
  `CLAUDE.md` / `AGENTS.md`、imports
  与 cwd / path 触发的 lazy-loaded instructions
  只适合注入廉价、快变的环境事实或局部约束；
  它不承载 canonical task truth、
  managed policy 或外部承诺
  - 若这类动态 surface
    会影响 tool choice、approval、写边界、
    恢复入口或对外写入，就必须能审计
    来源、命中条件、加载时机与回滚路径；
    不让“读了哪个文件 / 进入哪个目录”
    静默改写高风险行为基线
  - resume / compact / session restore
    可能再次触发同类加载；因此逻辑默认
    cheap、idempotent、re-entrant，
    external imports / linked rules 的路径解析、
    allow / deny、缓存与失效
    也不应藏成纯 UX 细节
...
27. deterministic gate 应贴近写路径；
    ...
    多组件任务可保留 partial credit
28. skill / subagent 默认只拿最小必要上下文，不继承全量 parent session；
    若 runtime 对未声明的 tool / permission scope 采用继承语义，
    omission 也视为 capability grant；大输出优先回写 artifact + handle
...
33. `MCP roots`、OAuth scopes、
    tool allowlists、permission mode 属于 capability grant /
    operational boundary，不是单独充分的安全边界；应显式配置与审计
34. remote MCP / connector 返回的
    URL、handle、download target 默认是不可信输出；
    通过 allowlist / policy 后才能跨边界消费
35. wait 也是状态；外部等待必须显式落成
    wakeup handle + deadline 的 transition。
    审批、中断与 async callback 带 stable id + version marker，
    resume 按 ID 配对；若协议已有
    task object / background handle，就复用
    原生 task id / cursor / cancel handle。
    `roots/list` 等 server-to-client request
    只属于 request scope
...
40. `SKILL.md`、subagent prompt、prompt object
    及其 metadata / frontmatter 默认属于 steerability surface，不是 enforced policy
...
47. `AGENTS.md`、`CLAUDE.md`、provider config / hooks、
    custom commands / MCP prompts 默认是 projection surface，
    不是多本并行 canonical truth
48. dynamic hook / session-start context
    与 path-scoped instruction loading 只放
    快变环境事实或局部约束；
    若影响恢复、审批或外部承诺，必须回写
    canonical task surface。resume / compact
    可能重触发，因此默认 cheap、idempotent、
    re-entrant，且来源与加载时机应可审计
49. `MCP roots` 用于 scope / discovery 与 operational boundary；
...
55. MCP token 必须 audience-bound，且不得 passthrough 给上游 API；
    敏感授权默认走 host-managed auth 或 out-of-band elicitation
56. trace correlation 最好作为协议元数据透传，例如 MCP `traceparent` /
    `baggage` 与 `tool_use` / `tool_result` `_meta`
