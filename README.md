...
6. 会驱动 tool call、state transition、
   ...
   不让自由文本直接支配下一跳
7. approvals / human gates 属于 trust-boundary control，不属于产品叙事层
8. capability grant 应 progressive：先 read、后 write、再 escalate；先窄 tool group、再高风险全局权限
9. remote auth / OAuth token / MCP credential / API secret
